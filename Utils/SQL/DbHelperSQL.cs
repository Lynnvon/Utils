...
namespace Utils.SQL
{
    public abstract class DbHelperSQL
    {
        ...
        public DbHelperSQL() { }
        public static void ExectSql(string databaseName, string backFileName, int status)
        {
            string backString = "BACKUP DATABASE " + databaseName.Trim() +
                                    "\n to disk='" + backFileName.Trim() + "'" +
                                    "\n  WITH FORMAT";
            string restoreString = " RESTORE DATABASE " + databaseName.Trim() +
                                   " \n FROM disk = '" + backFileName.Trim() + "' " +
                                    "\n  WITH REPLACE";
            SqlConnection conn = new SqlConnection();
            //conn.ConnectionString = System.Configuration.ConfigurationManager.AppSettings["connectionString"]; //"Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=master;Data Source=.";
            ...
            try
            {
                ...
                if (status == 1)
                {
                    comm.CommandText = backString;
                }
                else
                {
                    comm.CommandText = restoreString;
                }
                comm.CommandTimeout = 200000;
