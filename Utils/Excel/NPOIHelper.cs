...
namespace Utils.Excel
{
    public static class NPOIHelper
    {
        ...
        public static DataTable ImportExceltoDt(string strFileName)
        {
            ...
            IWorkbook workbook;
            ISheet sheet;
            using (FileStream file = File.Open(strFileName, FileMode.Open, FileAccess.Read))
            {
        ...
        /// <summary>
        /// 将指定sheet中的数据导出到datatable中 xls及xlsx都可以
        /// </summary>
        /// <param name="sheet">需要导出的sheet</param>
        /// <param name="HeaderRowIndex">列头所在行号，-1表示没有列头</param>
        /// <returns></returns>
        static DataTable TranslateDt(ISheet sheet, int HeaderRowIndex, bool needHeader)
        {
            DataTable table = new DataTable();
            IRow headerRow;
            int cellCount;
